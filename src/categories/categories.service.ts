...
export class CategoriesService {
  ...
  async create(createCategorieDto: CreateCategorieDto): Promise<Categorie> {
    ...
    const categorieFound = await this.categorieModel
      .findOne({ categorie })
      .exec();
  ...
  async getOne(categorie: string): Promise<Categorie> {
    ...
    if (!categorieFound) {
      throw new NotFoundException(
        `Categoria: ${categorie} inválida. Nenhuma categoria foi encontrada!`,
      );
    }
  ...
  async delete(_id: string): Promise<void> {
    ...
    if (!categorie) {
      throw new NotFoundException(
        `id: ${_id} inválido. Nenhuma categoria foi encontrada!`,
      );
    }
  ...
  async update(
    ...
    await this.categorieModel
      .findOneAndUpdate({ _id }, { $set: updateCategorieDto })
      .exec();
  }
  ...
  async linkCategoriePlayer(params: string[]): Promise<void> {
    ...
    const categoriePlayer = await this.categorieModel
      .find({ categorie })
      .where('players')
      .in(playerId);
    ...
    if (categoriePlayer.length > 0) {
      throw new BadRequestException(
        `Jogador: ${playerId} já está cadastrado na Categoria ${categorie}.`,
      );
    }
    ...
    await this.categorieModel
      .findOneAndUpdate({ categorie }, { $set: categorieFound })
      .exec();
  }
}
